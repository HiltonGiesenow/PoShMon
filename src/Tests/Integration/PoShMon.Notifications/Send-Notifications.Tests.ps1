...
Describe "Send-Notifications" {
    ...
    It "Should mark failures as Critical" {
        ...
        Mock -CommandName Send-PoShMonEmail -Verifiable -MockWith { return } -ParameterFilter { $Critical -eq $true }
        Mock -CommandName Send-PushbulletMessage -Verifiable -MockWith { return } -ParameterFilter { $Critical -eq $true }

        $actual = Send-Notifications $poShMonConfiguration $poShMonConfiguration.Notifications.Sinks $poShMonConfiguration.Notifications.When $testMonitoringOutput $totalElapsedTime -Verbose
    ...
    It "Should mark non-failures as NOT Critical" {
        ...
        Mock -CommandName Send-PoShMonEmail -Verifiable -MockWith { return } -ParameterFilter { $Critical -eq $false }
        Mock -CommandName Send-PushbulletMessage -Verifiable -MockWith { return } -ParameterFilter { $Critical -eq $false }
