...
Invoke-Pester -Path "$ProjectRoot\Tests\Unit" -OutputFormat NUnitXml -OutputFile "$ProjectRoot\RawUnitTestResults.xml" -PassThru | `
            Export-Clixml -Path "$ProjectRoot\PesterUnitTestResults.xml"

Invoke-Pester -Path "$ProjectRoot\Tests\Integration" -OutputFormat NUnitXml -OutputFile "$ProjectRoot\RawIntegrationTestResults.xml" -PassThru | `
